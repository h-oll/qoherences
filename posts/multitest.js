...
let zoom = 10;     
let start_of_session = Date.now();

/////////////////////// MULTI STATS /////////////////////////////////
let show_multi_stats = (data, selector, stat_fun) => {
	...
	div.transition()
            .duration(200)
            .style("opacity", .9);
	div.html(`Valeur: ${(stat_fun(d.data)).toFixed(2)}`)
...
`;
	...
	show_multi_stats(all_data, "#multi_stats_all", success_fun);
	show_multi_stats(session_data, "#multi_stats_session", success_fun);

	show_multi_stats(all_data, "#multi_time_stats_all", time_fun);
	show_multi_stats(session_data, "#multi_time_stats_session", time_fun);
